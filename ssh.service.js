...
function executeOverSSH(sshClient, cmd, { endConnectionAfter = true } = {}){
  return new Promise((res, rej) => {
    sshClient.exec(cmd, (error, channel) => {
      ...
      // handleChildProcess can be used here because the SSH Stream has the same methods and the same events as the child process
      handleChildProcess(channel, { finishSignal: 'close' }).then(res).catch(rej)
    })
  })
}
